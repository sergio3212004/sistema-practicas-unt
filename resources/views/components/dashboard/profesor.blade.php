...
<section class="rounded-2xl bg-blue-950 p-6 text-white shadow-panel sm:p-7">
    <div class="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <h2 class="mt-2 text-2xl font-bold tracking-tight">Bienvenido, {{ $profesor->user->nombre }}</h2>
        ...
        <div class="rounded-xl border border-white/15 bg-white/10 px-4 py-3">
            ...
            <p class="mt-0.5 text-sm font-bold text-white">{{ $profesor->codigo_profesor }}</p>
        </div>
    </div>
</section>

<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
    <x-ui.stat-card label="Aulas asignadas" :value="$aulas->count()" description="Grupos bajo supervisión" icon="heroicon-o-academic-cap" />
    <x-ui.stat-card label="Estudiantes" :value="$aulas->sum(fn ($aula) => $aula->alumnos->count())" description="En todas tus aulas" icon="heroicon-o-users" />
    <x-ui.stat-card label="Actividades activas" :value="$actividadesActivas" description="Dentro del plazo" icon="heroicon-o-clipboard-document-check" tone="warning" />
    <x-ui.stat-card label="Entregas recibidas" :value="$totalEntregas" description="Trabajos registrados" icon="heroicon-o-inbox-arrow-down" tone="success" />
</div>

<section>
    <div class="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <p class="ui-eyebrow">Periodo {{ $semestreActivo?->nombre ?? 'sin activar' }}</p>
    ...
    @if($aulas->isNotEmpty())
        <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            @foreach($aulas as $aula)
                @php($entregasAula = $aula->actividades->sum(fn ($actividad) => $actividad->entregas->count()))
                <a href="{{ route('profesor.aulas.show', $aula) }}" class="ui-card group overflow-hidden transition hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-raised">
                    <div class="border-b border-gray-100 bg-blue-900 px-5 py-4 text-white">
                        <div class="flex items-center justify-between gap-4">
                            <div>
                                <p class="text-[10px] font-bold uppercase tracking-[0.16em] text-gold-400">{{ $aula->semestre->nombre ?? 'Semestre' }}</p>
                                <h4 class="mt-1 text-xl font-bold">Aula {{ $aula->numero }}</h4>
                            </div>
                    ...
                    <div class="p-5">
                        <dl class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <dd class="mt-1 text-lg font-bold text-gray-950">{{ $aula->alumnos->count() }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium text-gray-500">Semanas</dt>
                                <dd class="mt-1 text-lg font-bold text-gray-950">{{ $aula->semanas->count() }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium text-gray-500">Actividades</dt>
                                <dd class="mt-1 text-lg font-bold text-gray-950">{{ $aula->actividades->count() }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium text-gray-500">Entregas</dt>
                                <dd class="mt-1 text-lg font-bold text-gray-950">{{ $entregasAula }}</dd>
                            </div>
                        </dl>
