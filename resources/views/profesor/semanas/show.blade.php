<x-app-layout :title="'Semana '.$semana->numero">
    <x-slot name="header">
        <x-ui.page-header eyebrow="Planificación académica" :title="'Semana '.$semana->numero.($semana->nombre ? ' · '.$semana->nombre : '')" :description="'Aula '.$semana->aula->numero.' · '.($semana->aula->semestre?->nombre ?? 'Sin semestre')" icon="heroicon-o-calendar-days">
            <x-slot name="actions">
                ...
                <a href="{{ route('profesor.actividades.create', ['aula' => $semana->aula, 'semana' => $semana]) }}" class="ui-btn-primary">@svg('heroicon-o-plus', 'h-4 w-4') Nueva tarea</a>
            </x-slot>
        </x-ui.page-header>
    </x-slot>
    ...
    <div class="ui-page">
        <div class="grid gap-4 sm:grid-cols-3">
            <x-ui.stat-card label="Tareas" :value="$metricas['actividades']" description="Planificadas esta semana" icon="heroicon-o-clipboard-document-list" />
        ...
        <section class="ui-card overflow-hidden">
            <div class="ui-card-header">
                <x-ui.section-heading title="Tareas de la semana" :description="$metricas['actividades'].' tareas registradas.'" icon="heroicon-o-list-bullet">
            ...
            @if($semana->actividades->isEmpty())
                <div class="p-5 sm:p-6"><x-ui.empty-state title="La semana aún no tiene tareas" description="Crea la primera tarea para indicar objetivos, instrucciones y plazo." icon="heroicon-o-clipboard-document-list"><x-slot name="actions"><a href="{{ route('profesor.actividades.create', ['aula' => $semana->aula, 'semana' => $semana]) }}" class="ui-btn-primary">Crear tarea</a></x-slot></x-ui.empty-state></div>
            @else
        ...
        <section class="rounded-xl border border-red-200 bg-red-50 p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div><h2 class="font-bold text-red-900">Eliminar semana</h2><p class="mt-1 text-sm text-red-800">También eliminará sus tareas y entregas asociadas.</p></div>
                <form action="{{ route('profesor.semanas.destroy', $semana) }}" method="POST" onsubmit="return confirm('¿Eliminar esta semana y todas sus tareas? Esta acción no se puede deshacer.');">@csrf @method('DELETE')<button type="submit" class="ui-btn-danger">@svg('heroicon-o-trash', 'h-4 w-4') Eliminar</button></form>
            </div>
        </section>
    </div>
</x-app-layout>
